...
fun ShowSimpleForm(book: MBook, navController: NavHostController) {
    ...
    Row {
        ...
        val isFinishedTimeStamp =
            if (isFinishedReading.value) com.google.firebase.Timestamp.now() else book.finishedReading
        val isStartedTimeStamp =
            if (isStartedReading.value) com.google.firebase.Timestamp.now() else book.startedReading
        ...
        RoundedButton(label = "Update") {
            if (bookUpdate) {
                book.id?.let {
                    FirebaseFirestore.getInstance().collection("books").document(it)
                        .update(bookToUpdate).addOnCompleteListener {
                            ...
                            Log.d("Error", "ShowSimpleForm: ,Error updating document", it)
                        }
                }
            }
        }
        ...
        if (openDailog.value) {
            ShowAlertDailog(message = stringResource(id = R.string.sure) + "\n" + stringResource(id = R.string.action),openDailog){
...
@Composable
fun ShowAlertDailog(message: String, openDailog: MutableState<Boolean>, onYesPressed: () -> Unit) {
        ...
        AlertDialog(
            ...
            confirmButton = {
                Row(modifier=Modifier.padding(all=8.dp), horizontalArrangement = Arrangement.Center){
                    ...
                    TextButton(onClick = {openDailog.value = false}) {
                        Text(text = "No")
                    }
                }
            })
